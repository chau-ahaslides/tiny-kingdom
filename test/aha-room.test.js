...
test('auto: decide() picks relay, page and role from where the page runs', () => {
  const { decide } = AhaRoom;
  // the game site itself: same-origin relay, path as page, host unless a join code is present
  let d = decide({ origin: 'https://play.ahaslides.io', hostname: 'play.ahaslides.io', pathname: '/idea-map', search: '' });
  assert.deepEqual([d.relay, d.page, d.role, d.join], ['https://play.ahaslides.io', '/idea-map', 'host', null]);
  d = decide({ origin: 'https://play.ahaslides.io', hostname: 'play.ahaslides.io', pathname: '/idea-map', search: '?join=ab12' });
  assert.deepEqual([d.role, d.join], ['audience', 'AB12']);
  // wrangler dev and workers.dev are the relay too
  d = decide({ origin: 'http://localhost:8787', hostname: 'localhost', pathname: '/x' });
  assert.equal(d.relay, 'http://localhost:8787');
  d = decide({ origin: 'https://tiny-kingdom.ahaslides-game.workers.dev', hostname: 'tiny-kingdom.ahaslides-game.workers.dev', pathname: '/x' });
  assert.equal(d.relay, 'https://tiny-kingdom.ahaslides-game.workers.dev');
  // a page on another AhaSlides site: relay is play, page is its own URL, join from the hash works
  d = decide({ origin: 'https://agent-fleet.ahaslides.io', hostname: 'agent-fleet.ahaslides.io', pathname: '/artifacts/abc', hash: '#join=zz99' });
  assert.deepEqual([d.relay, d.page, d.role, d.join], ['https://play.ahaslides.io', 'https://agent-fleet.ahaslides.io/artifacts/abc', 'audience', 'ZZ99']);
  // sandboxed: no location to trust; window.name or the parent's message supply page and code
  d = decide({ origin: 'null', name: 'aha:{"url":"https://agent-fleet.ahaslides.io/artifacts/abc"}' });
  assert.deepEqual([d.relay, d.page, d.role, d.sandboxed], ['https://play.ahaslides.io', 'https://agent-fleet.ahaslides.io/artifacts/abc', 'host', true]);
  d = decide({ origin: 'null', parent: { url: 'https://agent-fleet.ahaslides.io/artifacts/abc', join: 'q7q7' } });
  assert.deepEqual([d.role, d.join, d.page], ['audience', 'Q7Q7', 'https://agent-fleet.ahaslides.io/artifacts/abc']);
  d = decide({ origin: 'null', referrer: 'https://agent-fleet.ahaslides.io/artifacts/abc?x=1#y' });
  assert.equal(d.page, 'https://agent-fleet.ahaslides.io/artifacts/abc?x=1');
  d = decide({ origin: 'null', referrer: 'https://agent-fleet.ahaslides.io/' });
  assert.equal(d.page, null, 'a bare origin referrer is not a page');
  // explicit options win
  d = decide({ origin: 'null', opts: { page: 'https://x.ahaslides.ai/p', origin: 'https://r.ahaslides.io', role: 'host', code: 'IGN0' } });
  assert.deepEqual([d.relay, d.page, d.role], ['https://r.ahaslides.io', 'https://x.ahaslides.ai/p', 'host']);
});

test('auto: hosts or joins on the memory hub by role', async () => {
  const hub = AhaRoom.memory();
  const h = await AhaRoom.auto({ transport: hub, page: '/t', role: 'host' });
  assert.equal(h.role, 'host'); assert.ok(h.room);
  const a = await AhaRoom.auto({ transport: hub, role: 'audience', name: 'A' });
  assert.equal(a.role, 'audience'); assert.ok(a.me);
  await hub.settle();
  assert.equal(h.room.players.size, 1);
  h.room.close?.(); a.me.close?.();
});
